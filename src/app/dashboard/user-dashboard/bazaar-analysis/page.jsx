...
export default function UserBazaarAnalysisPage() {
  ...
  const [bazaars, setBazaars] = useState(() => {
    if (typeof window !== "undefined" && userId) {
      const cached = sessionStorage.getItem(`usr_bazaar_analysis_${userId}_${currentMonth}_${currentYear}`);
      if (cached) {
        try {
  ...
  const loadData = useCallback(async () => {
    ...
    const cacheKey = `usr_bazaar_analysis_${userId}_${month}_${year}`;
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/bazaars/user/${userId}?month=${month}&year=${year}`
      );
  ...
  }, [userId, month, year, bazaars.length]);
